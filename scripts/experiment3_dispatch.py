...
TASK_FILE = Path("tasks/textproc/task_description.json")
...
def plot_results(rows: list[dict], out_dir: Path, n: int, k: int, provider: str, model: str) -> Path:
    ...
    fig.suptitle(
        f"textproc Experiment — Dynamic vs Fixed-N vs Static Dispatch\n"
        f"n={n} teammates · up to {k} trials per condition · {provider} / {model}\n"
        f"Task: textproc (text processing library extension)",
    ...
    out_path = out_dir / "textproc_results.png"
...
def print_summary(rows: list[dict]):
    ...
    print(f"\n{'═' * 66}")
    print(f"  TEXTPROC EXPERIMENT RESULTS")
    print(f"{'═' * 66}")
...
def main():
    parser = argparse.ArgumentParser(description="textproc Experiment: Dynamic vs Fixed-N Dispatch")
    ...
    parser.add_argument(
        ...
        default=None,
        help="Comma-separated conditions to run (default: static,dynamic,no_graph,decentralized,metagpt). "
    ...
    exp_dir = Path("experiments") / "experiment3" / f"n{args.n}"
    ...
    print(f"\n{'═' * 62}")
    print(f"  TEXTPROC EXPERIMENT — Dynamic vs Fixed-N Dispatch")
    print(f"{'═' * 62}")
    print(f"  task            : textproc (text processing library extension)")
    ...
    with csv_path.open(csv_mode, newline="") as csv_file:
        ...
        active_conditions = args.conditions or ["dynamic", "decentralized", "static", "no_graph", "metagpt"]
